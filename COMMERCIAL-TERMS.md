Commercial Terms

1. Prohibited Commercial Use
   Commercial use of this software is prohibited without explicit permission
   from the author. "Commercial use" includes, but is not limited to:

   - selling the software or modified versions;
   - offering paid access to the software;
   - using the software to provide paid pentesting, consulting, or SaaS;
   - integrating the software into a paid product or service;
   - using the software in any business activity that generates revenue.

2. Allowed Use
   Non-commercial, personal, research, experimental and educational uses are
   allowed under the Apache 2.0 license.

3. How to Request Commercial Use
   To request commercial rights, open a GitHub Issue labeled
   "Commercial request". Do not send email.

4. Liability
   These terms do not modify the warranty disclaimer or liability limitations
   in Apache 2.0. The software is provided AS IS.

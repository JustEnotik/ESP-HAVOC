...
void beaconSpamScreen(uint8_t selectedItem) {
  ...
  while (true) {
    ...
    switch (selectedItem) {
      ...
      case 2: {
          const char* tempSSID[] = {randomSSID()};
          handleBeaconSpam(tempSSID, 1);
          break;
        }
    ...
    bufferSpamScreen(
      ...
      beaconSpamCurrentAP.c_str());
    u8g2.sendBuffer();
  }
}
void bluetoothSpamScreen(uint8_t selectedItem) {
  lastUpDebounceTime = millis();
  lastDownDebounceTime = millis();
  lastSelectDebounceTime = millis();
  lastBackDebounceTime = millis();
  ...
  switch (selectedItem) {
    case 1: /* setupAppleBluetoothSpam(); */ break;
    case 2: break;
    case 3: break;
  }
  ...
  while (true) {
    ...
    switch (selectedItem) {
      case 1: /* handleAppleBluetoothSpam(); */ break;
      case 2: break;
      case 3: break;
    }
    ...
    switch (selectedItem) {
      case 1:
        /* bufferSpamScreen(
          u8g2,
          BLUETOOTH_MENU_ITEMS[selectedItem],
          appleBluetoothSpamCurrentDevice()); */ break;
      case 2: break;
      case 3: break;
    }
    u8g2.sendBuffer();
...
void aboutScreen(
  ...
  while (true) {
    ...
    if (millis() - lastFrameTime >= 1000 / OLED_FPS) {
      ...
      u8g2.drawXBM(4, 49, 66, 11, bitmap_boot_logo);

      u8g2.setFont(UI_FONT);
      u8g2.drawStr(4, 11, (FIRMWARE_VERSION).c_str());

      u8g2.drawStr(4, 20, ("Build Date: " + BUILD_DATE).c_str());

      u8g2.drawStr(4, 30, "Don't be a skid");
      u8g2.sendBuffer();
    }
  }
}
  ...
  if (digitalRead(BUTTON_SELECT) == LOW && (millis() - lastSelectDebounceTime) > DEBOUNCE_DELAY) {
    if (millis() - lastActionTime < STANDBY_TIMEOUT) {
      switch (selectedItem) {
        case 0:
          ...
          [](uint8_t wifi_selected) {
            switch (wifi_selected) {
              ...
              case 2: packetMonitorMenu(); break;
              case 3: break;
            }
          }
          ); break;
        ...
        case 2: break;
        case 3:
          aboutScreen(FIRMWARE_VERSION, BUILD_DATE); break;
        case 4:
          listMenu(
            SETTINGS_MENU_ITEMS,
            SETTINGS_MENU_ICONS,

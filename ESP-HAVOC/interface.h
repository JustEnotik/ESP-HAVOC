...
void bufferSpamScreen(
  U8G2& u8g2, 
  const char* spamName,
  const char* spamLog) {
  // Текст
  u8g2.drawStr((128 - (strlen((String(spamName)).c_str()) * 6)) / 2, 27, (String(spamName)).c_str());
  u8g2.drawStr((128 - (strlen((String(spamLog)).c_str()) * 6)) / 2, 36, (String(spamLog)).c_str());

  // Назад
  u8g2.drawStr(57, 46, "Back");

  // Иконка назад
  u8g2.drawXBM(47, 38, 8, 8, bitmap_small_back_icon);
}
